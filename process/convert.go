...
func (p Convert) ApplyBatch(ctx context.Context, caps []config.Capsule) ([]config.Capsule, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("convert applybatch: %v", err)
	}
...
func (p Convert) Apply(ctx context.Context, cap config.Capsule) (config.Capsule, error) {
	...
	if p.Options.Type == "" {
		return cap, fmt.Errorf("convert apply: options %+v: %v", p.Options, errProcessorMissingRequiredOptions)
	}
	...
	if p.InputKey != "" && p.OutputKey != "" {
		...
		if err := cap.Set(p.OutputKey, value); err != nil {
			return cap, fmt.Errorf("convert apply: %v", err)
		}

		return cap, nil
	}

	return cap, fmt.Errorf("convert apply: inputkey %s outputkey %s: %v", p.InputKey, p.OutputKey, errProcessorInvalidDataPattern)
}
