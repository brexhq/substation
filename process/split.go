...
func (p Split) ApplyBatch(ctx context.Context, caps []config.Capsule) ([]config.Capsule, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("split applybatch: %v", err)
	}
	...
	for _, cap := range caps {
		...
		return nil, fmt.Errorf("split applybatch: inputkey %s outputkey %s: %v", p.InputKey, p.OutputKey, errProcessorInvalidDataPattern)
	}
...
func (p Split) Apply(ctx context.Context, cap config.Capsule) (config.Capsule, error) {
	...
	if p.Options.Separator == "" {
		return cap, fmt.Errorf("split apply: options %+v: %v", p.Options, errProcessorMissingRequiredOptions)
	}

	// only supports JSON, error early if there are no keys
	if p.InputKey == "" || p.OutputKey == "" {
		return cap, fmt.Errorf("split apply: inputkey %s outputkey %s: %v", p.InputKey, p.OutputKey, errProcessorInvalidDataPattern)
	}
	...
	if err := cap.Set(p.OutputKey, value); err != nil {
		return cap, fmt.Errorf("split apply: %v", err)
	}
