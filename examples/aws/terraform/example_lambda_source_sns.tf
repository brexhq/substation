...
module "lambda_sns_source" {
  ...
  env = {
    "AWS_MAX_ATTEMPTS" : 10
    "AWS_APPCONFIG_EXTENSION_PREFETCH_LIST" : "/applications/substation/environments/prod/configurations/substation_sns_source"
...
resource "aws_sns_topic_subscription" "lambda_subscription_sns_source" {
  topic_arn = module.sns_source.arn
  protocol  = "lambda"
  endpoint  = module.lambda_sns_source.arn

  depends_on = [
    module.lambda_sns_source.name
  ]
}
...
resource "aws_lambda_permission" "lambda_sns_source_invoke" {
  statement_id  = "AllowExecutionFromSNS"
  action        = "lambda:InvokeFunction"
  function_name = module.lambda_sns_source.name
  principal     = "sns.amazonaws.com"
  source_arn    = module.sns_source.arn
