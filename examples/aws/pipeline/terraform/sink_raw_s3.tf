...
module "lambda_raw_s3_sink" {
  ...
  env = {
    "AWS_MAX_ATTEMPTS" : 10
    "AWS_APPCONFIG_EXTENSION_PREFETCH_LIST" : "/applications/substation/environments/prod/configurations/substation_raw_s3_sink"
...
resource "aws_lambda_event_source_mapping" "lambda_esm_raw_s3_sink" {
  event_source_arn                   = module.kinesis_raw.arn
  function_name                      = module.lambda_raw_s3_sink.arn
