...
func (p Flatten) ApplyBatch(ctx context.Context, caps []config.Capsule) ([]config.Capsule, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("flatten applybatch: %v", err)
	}
...
func (p Flatten) Apply(ctx context.Context, cap config.Capsule) (config.Capsule, error) {
	...
	if p.InputKey == "" && p.OutputKey == "" {
		return cap, fmt.Errorf("flatten apply: inputkey %s outputkey %s: %v", p.InputKey, p.OutputKey, errProcessorInvalidDataPattern)
	}
	...
	if err := cap.Set(p.OutputKey, value); err != nil {
		return cap, fmt.Errorf("flatten apply: %v", err)
	}
